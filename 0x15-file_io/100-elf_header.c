 ...
 * @e_ident: its a pointer to an array containing the ELF magic no.
 *
...
void check_elf(unsigned char *e_ident)
{
	...
	for (postion = 0; postion < 4; postion++)
	{
		if (e_ident[postion] != 127 &&
		    e_ident[postion] != 'E' &&
		    e_ident[postion] != 'L' &&
		    e_ident[postion] != 'F')
		{
...
void print_magic(unsigned char *e_ident)
{
	...
	for (postion = 0; postion < EI_NIDENT; postion++)
	{
		printf("%02x", e_ident[postion]);

		if (postion == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
 ...
 * @e_ident: its a pointer to an array containing the ELF class.
 */
void print_class(unsigned char *e_ident)
{
	printf("  Class:                             ");
